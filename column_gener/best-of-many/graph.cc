...
void printEdges(Graph &g){
    cout<<"All Edges..."<<endl;
    for(std::tr1::unordered_map<string, Edge> :: const_iterator iter = g.edges.begin(); iter != g.edges.end(); iter++){
...
double pushRelabel(Graph &g, int source_id, int sink_id){
    ...
    setHeight(g, source_id, (int)g.vertices.size());
